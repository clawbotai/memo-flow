...
**更新** 项目已完成导航系统的重大重构，将首页和播客转录功能分离到独立的路由，实现了更清晰的页面结构和更好的用户体验。
...
subgraph "页面层"
HomePage[page.tsx - 首页]
PodcastPage[podcast/page.tsx - 播客转录]
TranscriptionsPage[transcriptions/page.tsx - 转录历史]
end
subgraph "服务层"
API[API 路由]
History[转录历史管理]
...
**更新** 导航系统现已支持三个主要页面：首页 (`/`)、播客转录 (`/podcast`) 和转录历史 (`/transcriptions`)，每个页面都有独立的路由映射和激活状态管理。
...
CheckType --> |首页| HomeRoute[跳转到 / 根路由]
CheckType --> |播客转录| PodcastRoute[跳转到 /podcast 路由]
CheckType --> |转录历史| HistoryRoute[跳转到 /transcriptions 路由]
CheckType --> |其他| StateNav[更新状态]
HomeRoute --> CloseMobile[关闭移动端菜单]
PodcastRoute --> CloseMobile
HistoryRoute --> CloseMobile
...
**更新** 导航系统现已支持三个主要路由：
- 首页：`/` - 对应 `home` 页面 ID
- 播客转录：`/podcast` - 对应 `podcast` 页面 ID  
- 转录历史：`/transcriptions` - 对应 `history` 页面 ID
...
AppShell->>AppShell : 根据路由更新 activePage
...
**更新** AppShell 现在能够正确识别三个主要路由：
- `/` - 设置 `activePage = 'home'`
- `/podcast` - 设置 `activePage = 'podcast'`
- `/transcriptions` - 设置 `activePage = 'history'`
...
**更新** 项目已完成重要的导航系统重构，实现了以下主要成就：

1. **模块化设计**：清晰的组件职责分离，便于维护和扩展
2. **响应式优先**：从移动端到桌面端的完整适配
3. **性能优化**：合理的渲染策略和资源管理
4. **用户体验**：直观的交互设计和即时的视觉反馈
5. **路由分离**：首页与播客转录功能的独立路由管理

新的导航系统不仅满足了当前的功能需求，还为未来的功能扩展奠定了坚实的基础。通过持续的优化和改进，MemoFlow 将继续为用户提供卓越的播客转录体验。