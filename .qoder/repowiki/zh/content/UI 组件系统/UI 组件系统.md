...
- 更新侧边栏导航简化后的结构和功能
- 完善设置面板的标签页分区和内容组织
- **新增** 转录详情组件智能复制功能，支持纯文本和带时间戳两种复制模式，以及下拉菜单交互
...
transdetail["TranscriptionDetail<br/>转录详情视图<br/>智能复制功能"]
end
subgraph "转录历史系统"
...
TranscriptionCard --> TranscriptionDetail["转录详情页面<br/>/transcriptions/[id]<br/>智能复制功能"]
...
- [src/components/transcription-detail.tsx:44-488](file://src/components/transcription-detail.tsx#L44-L488)
...
- 设计模式
  ...
  - 受控组件模式，支持重新转录功能
  - **新增** 智能复制功能，支持纯文本和带时间戳两种复制模式
  - **新增** 下拉菜单交互，提供复制选项选择
- 数据结构与复杂度
  - 状态管理复杂度 O(1)，SSE 连接维护
  - **新增** 复制状态管理：copied、copyMenuOpen
  - **新增** 复制菜单引用管理：copyMenuRef
- 依赖链
  - 依赖 utils.cn、Progress、Badge、Button、FlowLoader
  - 使用 TranscriptionRecord 和 TranscribeSegment 类型
  - **新增** 使用 lucide-react 图标：Copy、Check、ChevronDown、FileText、Clock

```mermaid
sequenceDiagram
participant U as "用户"
participant TD as "TranscriptionDetail"
participant Menu as "复制菜单"
participant Clipboard as "剪贴板"
U->>TD : 点击复制按钮
TD->>Menu : 显示下拉菜单
U->>Menu : 选择复制模式
Menu->>Clipboard : 执行复制操作
Clipboard-->>U : 复制成功反馈
```

**图表来源**
- [src/components/transcription-detail.tsx:183-234](file://src/components/transcription-detail.tsx#L183-L234)
- [src/components/transcription-detail.tsx:388-424](file://src/components/transcription-detail.tsx#L388-L424)
...
Detail --> SmartCopy["智能复制功能"]
Live --> Update["UI 更新"]
Retranscribe --> Reset["重置状态"]
Reset --> Live
SmartCopy --> CopyOptions["复制选项菜单"]
CopyOptions --> PlainText["纯文本复制"]
CopyOptions --> WithTimestamp["带时间戳复制"]
```
...
Icons["lucide-react"] --> TranscriptionDetail["转录详情组件"]
```
...
- [src/components/transcription-detail.tsx:9](file://src/components/transcription-detail.tsx#L9)
...
- 异步与订阅
  ...
  - **新增** 智能复制功能使用 useCallback 优化回调性能
- 渲染优化
  - 将大型内容放入可滚动容器，避免布局抖动
  - 折叠高级设置以减少初始渲染
  - 转录历史卡片使用网格布局，响应式显示
  - **新增** 标签页分区减少单个面板的渲染负担
  - **新增** 复制菜单使用 useRef 优化 DOM 引用管理
- 组件复用
  - 通过统一的变体/尺寸体系提高组件复用率
- 内存管理
  - SSE 连接在组件卸载时自动清理
...
- **新增** 智能复制功能异常
  - 检查浏览器剪贴板 API 权限
  - 确认复制菜单的点击外部关闭事件监听器
  - 检查 handleCopyPlainText 和 handleCopyWithTimestamp 函数
- **新增** 复制菜单显示问题
  - 检查 copyMenuOpen 状态管理
  - 确认 copyMenuRef 引用是否正确设置
  - 检查 CSS 定位和 z-index 层级
- 样式冲突
...
### **新增** 智能复制功能最佳实践
- 使用纯文本复制模式适用于直接粘贴到文档编辑器
- 使用带时间戳复制模式适用于需要精确定位的场景
- 复制菜单提供直观的图标和文字说明
- 支持点击外部自动关闭菜单，提升用户体验
- 复制成功后提供视觉反馈（勾选图标）
- 确保在不同浏览器环境下剪贴板 API 的兼容性
- 处理复制失败的情况，提供错误提示