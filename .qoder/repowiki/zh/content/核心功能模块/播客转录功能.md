...
## 更新摘要
**所做更改**
- 新增了完整的路由重构：播客转录功能迁移到独立的 `/podcast` 路由
- 新增了重新转录功能，支持通过 `/api/retranscribe` API 重新处理已有转录记录
- 新增了实时转录流功能，通过 `/api/transcription-live` API 提供实时进度更新
- 新增了转录详情页面 `/transcriptions/[id]` 和转录历史页面 `/transcriptions`
- 完善了转录历史系统的持久化存储和管理机制
- 增强了前端导航结构和用户界面
...
## 简介
本功能允许用户粘贴小宇宙播客链接，系统自动提取音频并进行本地语音转写，最终生成可复制的转录文本，并提供音频播放与字数统计等信息。系统采用前后端分离架构：前端负责用户交互与状态管理，后端提供 REST API 与 SSE 流，底层通过 whisper.cpp 实现本地语音识别。新版本完全替换了原有的模拟转录系统，采用真实的 Whisper 语音转文本引擎，并引入了完整的转录历史记录功能、实时转录流和重新转录功能。
...
subgraph "前端页面与组件"
UI_Podcast["播客转录页面<br/>src/app/podcast/page.tsx"]
UI_Detail["转录详情页面<br/>src/app/transcriptions/[id]/page.tsx"]
UI_History["转录历史页面<br/>src/app/transcriptions/page.tsx"]
UI_Components["UI 组件<br/>Toast/FlowLoader"]
end
subgraph "业务逻辑"
...
## 架构总览
系统采用"前端路由 + 后端 API + 本地模型 + 历史记录"的架构模式。前端通过独立的 `/podcast` 路由处理播客转录，通过 `/transcriptions` 路由管理历史记录，后端调用小宇宙 API/页面解析获取音频 URL，下载音频至临时目录，调用 whisper.cpp 进行转写，返回转录文本与音频信息。新版本支持实时转录流、进度跟踪、完整的转录历史记录管理和重新转录功能。
...
### 路由系统与导航结构
- **独立播客路由**：`/podcast` 路由专门处理播客转录功能，提供完整的转录流程
- **历史记录路由**：`/transcriptions` 路由管理所有转录历史记录，支持列表查看和详情页
- **详情路由**：`/transcriptions/[id]` 路由展示单个转录任务的详细信息和实时进度
- **导航菜单**：侧边栏包含播客转录、转录历史、设置等功能入口
- **移动端适配**：支持汉堡菜单和响应式布局

```mermaid
flowchart TD
Start(["用户访问应用"]) --> RouteCheck{"检查路由"}
RouteCheck --> |"/podcast"| PodcastPage["播客转录页面"]
RouteCheck --> |"/transcriptions"| HistoryPage["转录历史页面"]
RouteCheck --> |"/transcriptions/[id]"| DetailPage["转录详情页面"]
RouteCheck --> |"/"| HomePage["首页"]
PodcastPage --> Nav["侧边栏导航"]
HistoryPage --> Nav
DetailPage --> Nav
HomePage --> Nav
Nav --> Settings["设置对话框"]
Nav --> Whisper["Whisper 配置"]
```

**图表来源**
- [src/components/sidebar.tsx:32-78](file://src/components/sidebar.tsx#L32-L78)
- [src/components/app-shell.tsx:12-42](file://src/components/app-shell.tsx#L12-L42)
- [src/app/transcriptions/[id]/page.tsx:30-93](file://src/app/transcriptions/[id]/page.tsx#L30-L93)

**章节来源**
- [src/components/sidebar.tsx:10-78](file://src/components/sidebar.tsx#L10-L78)
- [src/components/app-shell.tsx:12-42](file://src/components/app-shell.tsx#L12-L42)
- [src/app/transcriptions/[id]/page.tsx:1-93](file://src/app/transcriptions/[id]/page.tsx#L1-L93)
...
## 结论
该播客转录功能通过清晰的前后端分工与多策略的小宇宙集成，实现了从链接输入到转录结果展示的完整流程。新版本完全替换了原有的模拟转录系统，采用真实的 Whisper 语音转文本引擎，支持实时转录流、进度跟踪、完整的转录历史记录管理和重新转录功能。新增的路由系统提供了更好的用户体验，独立的 `/podcast` 路由专注于转录功能，`/transcriptions` 路由提供历史记录管理，`/transcriptions/[id]` 路由展示详细的转录信息。前端提供良好的用户体验，后端保证了稳定性与可扩展性。建议在生产环境中进一步完善并发控制、错误恢复与监控告警机制。
