...
系统实现了基于 Server-Sent Events (SSE) 的实时进度监控，为用户提供流畅的用户体验。

```mermaid
flowchart TD
Start([开始监控]) --> ValidateTaskId[验证任务ID]
ValidateTaskId --> CreateStream[创建SSE流]
CreateStream --> SetupInterval[设置轮询间隔]
SetupInterval --> ReadProgress[读取进度文件]
ReadProgress --> HasProgress{有进度数据?}
