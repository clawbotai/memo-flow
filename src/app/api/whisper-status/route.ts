...
import fs from 'fs';
import { getWhisperConfig, formatFileSize, inferModelName } from '@/lib/whisper-config';
...
export async function GET() {
  try {
    ...
    // 检查 whisper.cpp 是否安装
    const whisperInstalled = fs.existsSync(config.whisperPath);

    // 检查模型文件是否存在
    const modelInstalled = fs.existsSync(config.modelPath);
    ...
    // 推断模型名称（从路径中）
    const modelName = inferModelName(config.modelPath);
